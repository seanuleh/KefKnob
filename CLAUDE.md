...
- OTA upload failing → device must be on WiFi and booted. OTA is HTTP-based (`POST /update` on port 80, served by built-in `WebServer` + `Update.h`). Test with `curl http://deskknob.local/` — should return an HTML upload form. ArduinoOTA / espota.py was removed because UDP 3232 is unreliable on Arduino-ESP32 3.x + ESP32-S3 + macOS. **`deskknob.local` mDNS does not resolve from every host (e.g. the uleh server) — POST to the device IP directly instead** (find it in UniFi: client "Desktop Dash ESP32"; as of 2026-07-15 it was `192.168.1.61`). The KEF speaker is a separate DHCP client "KEF LSXII - Sean Desktop" (`192.168.1.217`, not a fixed reservation — verify the IP in `config_local.h` if KEF control breaks after a lease change)
- OTA resets the device ~64KB into the upload (`curl (56) Recv failure: Connection reset by peer`, device reboots and recovers) → the Core 0 mic task (continuous I2S + PSRAM) and networkTask (HTTP/HTTPS + PSRAM) race the flash writes and fault the update. Fixed by suspending both tasks on `UPLOAD_FILE_START` and resuming on END/error/abort (`ota_suspend_core0()`/`ota_resume_core0()` in `main.cpp`, `g_mic_task_handle` exposed from `mic_pdm`). **Firmware predating this fix cannot OTA past 64KB — recover with a one-time USB serial flash from the Mac**
