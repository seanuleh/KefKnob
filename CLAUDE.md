...
networkTask() — Loops every 50ms        reads g_mic_level, pushes to ring buf,
  ...
  └─ Every 1s:                              main_screen_update_power_source()
       kef_get_speaker_status()         Service g_haptic_event → drv2605_play()
       if USB source:                   (Core 1 only — I2C_NUM_0 shared with touch)
         spotify_get_now_playing()      Encoder callbacks set g_haptic_event = CLICK
       else:
         kef_get_player_data()
       kef_get_source()
...
- Album art / Spotify metadata gone on USB source → test the refresh token from uleh (POST `accounts.spotify.com/api/token` with `grant_type=refresh_token`). `invalid_grant` + `"Refresh token revoked"` (seen 2026-09-24) means Spotify revoked it early — not the 6-month expiry. **Headless re-auth** (there is no script in `config_local.h`): have Sean open `https://accounts.spotify.com/authorize?client_id=<SPOTIFY_CLIENT_ID>&response_type=code&redirect_uri=http%3A%2F%2F127.0.0.1%3A8888%2Fcallback&scope=user-read-currently-playing+user-modify-playback-state&show_dialog=true`, paste back the dead `127.0.0.1` callback URL, exchange the `code` (`grant_type=authorization_code`, same redirect_uri, Basic auth client_id:secret), write `refresh_token` into `config_local.h`, `pio run -e ota -t upload`
...
- OTA upload failing → device must be on WiFi and booted. OTA is HTTP-based (`POST /update` on port 80, served by built-in `WebServer` + `Update.h`). Test with `curl http://deskknob.local/` — should return an HTML upload form. ArduinoOTA / espota.py was removed because UDP 3232 is unreliable on Arduino-ESP32 3.x + ESP32-S3 + macOS. **`deskknob.local` mDNS does not resolve from every host (e.g. the uleh server) — POST to the device IP directly instead: the DeskKnob is at `192.168.1.90`** (as of 2026-07-15; find it in UniFi if it moves). ⚠️ Do NOT confuse it with `192.168.1.61` "Desktop Dash ESP32" — that is a *different* ESP32 whose OTA endpoint resets at exactly 64KB; POSTing firmware there wastes time chasing a phantom bug. The KEF speaker is a separate DHCP client "KEF LSXII - Sean Desktop" (`192.168.1.217`, not a fixed reservation — verify the IP in `config_local.h` if KEF control breaks after a lease change)
- OTA is hardened against Core 0 flash/PSRAM contention: the mic task and networkTask are suspended for the flash write (`ota_suspend_core0()`/`ota_resume_core0()` in `main.cpp`, `g_mic_task_handle` exposed from `mic_pdm`) and resumed on END/error/abort. Cheap insurance; not tied to a confirmed fault
