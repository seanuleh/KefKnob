...
void Touch_Init(void)
{
  i2c_config_t conf = 
  {
    ...
    .sda_io_num = TOUCH_SDA,         // Configure the GPIO of the SDA
    .scl_io_num = TOUCH_SCL,         // Configure GPIO for SCL
  ...
  uint8_t data = 0x00;
  I2C_writr_buff(TOUCH_I2C_ADDR,0x00,&data,1); //Switch to normal mode
}
uint8_t getTouch(uint16_t *x,uint16_t *y)
{
  uint8_t GetNum = 0;
  uint8_t data[7] = {0};
  I2C_read_buff(TOUCH_I2C_ADDR,0x00,data,7);
  GetNum = data[2];
  if(GetNum)
  {
